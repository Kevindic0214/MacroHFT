...
class episodicmemory():
    ...
    def add(self, hidden_state, action, q_value, single_state, trend_state, previous_action):
        ...
        if hasattr(q_value, "__len__") and len(q_value) > 0:
            self.buffer["q_value"][self.count] = q_value.item() if isinstance(q_value, torch.Tensor) else q_value[0]
        else:
    ...
    def query(self, query_hidden_state, action):
        if self.current_size != self.capacity:
            ...
            action_scalar = float(action) if hasattr(action, "__len__") else action
            mask = (top_k_actions == action_scalar).astype(float)
            weights = kernel_values[top_k_indices] / np.sum(kernel_values[top_k_indices])
            masked_weights = weights * mask
            
            sum_masked_weights = np.sum(masked_weights)
            if sum_masked_weights > 1e-10:  # 防止除以極小值
                normalized_weights = masked_weights / sum_masked_weights
