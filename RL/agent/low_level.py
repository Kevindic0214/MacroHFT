...
parser.add_argument("--epsilon_start", type=float, default=0.5)  # 保留以兼容，但Noisy Nets不使用
parser.add_argument("--epsilon_end", type=float, default=0.1)    # 保留以兼容
parser.add_argument("--decay_length", type=int, default=5)       # 保留以兼容
...
class subagent(nn.Module):
    def __init__(self, n_state_1, n_state_2, n_action, hidden_dim=64):
        super(subagent, self).__init__()
        self.n_state_1 = n_state_1
        self.n_state_2 = n_state_2
        
        def get_num_heads(embed_dim, max_heads=4):
            for h in range(max_heads, 0, -1):
                if embed_dim % h == 0:
                    return h
            return 1  
        
        num_heads_1 = get_num_heads(n_state_1)
        num_heads_2 = get_num_heads(n_state_2)
        
        self.attention_1 = nn.MultiheadAttention(embed_dim=n_state_1, num_heads=num_heads_1)
        self.fc1_1 = NoisyLinear(n_state_1, hidden_dim)
        
        self.attention_2 = nn.MultiheadAttention(embed_dim=n_state_2, num_heads=num_heads_2)
        self.fc1_2 = NoisyLinear(n_state_2, hidden_dim)
        
        self.fc2 = NoisyLinear(hidden_dim * 2 + 1, hidden_dim) 
        self.fc3 = NoisyLinear(hidden_dim, n_action)
        
    def forward(self, state, state_trend, previous_action):
        state = state.unsqueeze(0)  
        state_trend = state_trend.unsqueeze(0)  
        
        state, _ = self.attention_1(state, state, state)
        state = state.squeeze(0) 
        state_trend, _ = self.attention_2(state_trend, state_trend, state_trend)
        state_trend = state_trend.squeeze(0) 
        
        x1 = F.relu(self.fc1_1(state))
        x2 = F.relu(self.fc1_2(state_trend))
        if previous_action.dim() == 1:
            previous_action = previous_action.unsqueeze(-1)
        x = torch.cat([x1, x2, previous_action.float()], dim=-1)
        x = F.relu(self.fc2(x))
        return self.fc3(x)
    
    def reset_noise(self):
        self.fc1_1.reset_noise()
        self.fc1_2.reset_noise()
...
class DQN(object):
    def __init__(self, args):
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is not available. Ensure CUDA is installed and a GPU is present.")
        
        # 使用 args.device
        resolved_device_str = args.device
        if not resolved_device_str.startswith("cuda:") or not torch.cuda.is_available():
            print(f"Warning: Requested device {args.device} is not a CUDA device or CUDA is not available. Falling back to CPU if possible or first available CUDA device if format is incorrect.")
            if torch.cuda.is_available():
                # Attempt to parse the device index if format is like "cuda:X"
                try:
                    device_index = int(resolved_device_str.split(':')[-1])
                    if device_index >= torch.cuda.device_count():
                        print(f"Warning: CUDA device index {device_index} is out of range. Max index is {torch.cuda.device_count() - 1}. Falling back to cuda:0.")
                        resolved_device_str = "cuda:0"
                    else:
                        resolved_device_str = f"cuda:{device_index}" # Ensure correct format
                except ValueError: # Handle cases like "cuda" or other non-indexed formats
                    print(f"Could not parse device index from {resolved_device_str}. Falling back to cuda:0.")
                    resolved_device_str = "cuda:0" 
            else:
        ...
        try:
            ...
            if self.device.type == 'cuda':
                # This check is a bit redundant if the above logic correctly sets resolved_device_str
                # but kept for safety.
                if self.device.index is not None and self.device.index >= torch.cuda.device_count():
                    print(f"Warning: CUDA device index {self.device.index} is out of range after parsing. Max index is {torch.cuda.device_count() - 1}. Falling back to cuda:0.")
                    self.device = torch.device("cuda:0")
                # Ensure torch.cuda.set_device is called with an integer index if on CUDA
                current_cuda_device_index = self.device.index if self.device.index is not None else 0
                torch.cuda.set_device(current_cuda_device_index)
                print(f"Using device: {self.device} ({torch.cuda.get_device_name(current_cuda_device_index)})")
            else:
                print(f"Using device: {self.device}")
        except Exception as e:
            raise RuntimeError(
                f"Failed to initialize device {resolved_device_str}. Error: {e}. "
                "Ensure the GPU is accessible and CUDA drivers are compatible. "
                "Check 'nvidia-smi' for GPU status or set CUDA_VISIBLE_DEVICES."
            )

        self.result_path = os.path.join("./result/low_level", 
                                        '{}'.format(args.dataset), '{}'.format(args.clf), 
                                        str(int(args.alpha)), args.label) # args.label (string) is used here
        self.label_str = args.label # Store the original string label for consistent path creation
        self.label = int(args.label.split('_')[1]) # Keep the integer part for existing logic (e.g. indexing self.train_index)
        ...
        print("完整檔案路徑:", os.path.join(self.train_data_path, 'slope_labels.pkl'))
        if args.clf == 'slope':
        ...
        self.epsilon_scheduler = LinearDecaySchedule(start_epsilon=self.epsilon_start, 
    ...
    def update(self, replay_buffer):
        ...
        next_actions = self.eval_net(batch['next_state'], batch['next_state_trend'], 
                                    batch['next_previous_action']).argmax(dim=-1, keepdim=True)
        q_target = batch['reward'] + self.gamma * (1 - batch['terminal']) * self.target_net(
            batch['next_state'], batch['next_state_trend'], 
            batch['next_previous_action']).gather(-1, next_actions).squeeze(-1)
    ...
    def act(self, state, state_trend, info):
        ...
        previous_action = torch.tensor(info["previous_action"]).long().to(self.device).view(1, 1)  # 確保形狀為 [1, 1]
    ...
    def train(self):
        ...
        for sample in range(self.epoch_number):
            ...
            for i in range(df_number):
                ...
                while True:
                    ...
                    self.replay_buffer.store_transition(s, s2, info['previous_action'], 
                                                      info['q_value'], a, r, s_, s2_, 
                                                      info_['previous_action'], info_['q_value'], done)
            ...
            epoch_counter += 1
            self.epsilon = self.epsilon_scheduler.get_epsilon(epoch_counter)  # 保留以兼容
            ...
            self.scheduler.step(return_rate_eval) 
        ...
        # Corrected best_model_dir to be consistent with other paths and avoid overwriting
        # It will now include alpha, the original string label, and seed.
        # self.model_path is ./result/low_level/{dataset}/{clf}/{alpha}/{args.label}/seed_{seed}
        # We will save best_model.pkl inside this seed-specific directory.
        best_model_parent_dir = self.model_path 
        
        if not os.path.exists(best_model_parent_dir):
            os.makedirs(best_model_parent_dir)
        best_model_path = os.path.join(best_model_parent_dir, 'best_model.pkl')
        
        if best_model is not None: # Ensure best_model is not None before saving
            torch.save(best_model, best_model_path)
            print(f"Saved best model to {best_model_path}")
        else:
            print("No best model was found or saved during training.")
